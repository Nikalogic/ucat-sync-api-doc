...
(4.) Изображения одного продукта | GET | /manufacturers/{GLN-номер компании}/products/{GTIN}/images|Инфмация о доступных изображениях товара в разном разрешениях.
(5.) Передать статус по товару | POST | /response|Передать в каталог uCat статус полученного товара в системе клиента.
(6.) Поиск по оштрихкоду |GET|/products/{GTIN}/search|Поиск по товарам, которые доступны для вас в каталоге
(7.) Импорт артикулов| POST|/upload-sku|Для синхронизации асортимента, клиент может загрузить соответствие своих артикулов к штрихкодам (GTIN).
...
Запрос возвращает подробную иформацию о товарной позииции. Информация возвращается в иерархическом виде, вместе с упаковками
<pre>
Коробка
|
 -- Припак
...
* **{fields}** - необязательный параметр, содежит в себе список полей, которые должны быть в ответе по товару. ChildGTIN всегда будет пристуствовавть в ответе, для построения иерархии.
