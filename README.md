...
---------|-----|--------|------------
1. Список поставщиков в каталоге|GET|api-sync/v1/manufacturers|Работает без подписки. Выводить поставщиков зарегистрированных в каталоге
...
3. Информация об одном продукте|GET|api-sync/v1/manufacturers/[GLN-номер компании]/products/[GTIN]|Выдается информация о товаре и о его упаковках. Информация выдается в иерархическом виде. На верхнем уровне самая большая упаковка, на нижнем уровне сам товар.
4. Картинки одного продукта|GET|api-sync/v1/manufacturers/[GLN-номер компании]/products/[GTIN]/images|Инфмация о доступных изображениях товара в разном разрешениях.
5. Передать статус по полученному товару|POST|api-sync/v1/response|Передать в каталог uCat статус полученного товара в системе клиента.
6. Поиск товара по оштрихкоду|GET|api-sync/v1/products/[GTIN]/search|Поиск по товарам, которые доступны для вас в каталоге
7. Загрузкить файл с артикулами|POST|api-sync/v1/upload-sku|Для синхронизации асортимента, клиент может загрузить соответствие своих артикулов к штрихкодам (GTIN).
...
9. Получить подписки|GET|api-sync/v1/get-count-subscribe-product|Запрос возвращает перечень подписок, которые оформил клиент.
...
###Передать статус обработки товара торговой сетью###
...
```
*Пример ответов:*
1. Товар найден в каталоге по штрихкоду - вернется ответ с [информацией о товаре]()
1. Товар есть в каталоге, но торговая сеть не оформила на него подписку, API возвращает сообщение об ошибке
```javascript
{
  "code": "403 Forbidden",
...
```
1. Товара есь в каталоге но не опубликован на торговую сеть
```javascript
{
  "code" : 403 Forbidden, 
...
###Импорт артикулов торговой сети в базу uCat###


###Список изменившихся товаров###


###Количество подписок###


###Список доступных GTIN базовых единиц на которые была оформлена подписка###
